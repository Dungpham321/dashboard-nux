...
export function buildFormItem(config: Record<string, any>): Record<string, any> {
  ...
  if (typeof config.t === 'function') {
    item.template = config.t
  }
  return item
}

export function createFormOptions(overrides: Partial<any> = {}): Record<string, any> {
  const baseOptions = {
    colCount: 2,
    readOnly: false,
