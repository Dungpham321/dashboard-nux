<template>
    <div v-if="isReady">
        <BaseDataTree :grid-key="gridkey" :data-source="dataSource.data" :cols="Columns"
...
import BaseDataTree from '~/components/devextreme/BaseDataTree.vue';
...
const gridkey = ref('danhmucitemtree');
...
const baseTreeRef = useState('gridRef-' + gridkey.value);
const Columns = [];
onBeforeMount(async () => {
    ...
    if (AccessRights.danhmuc.PID) {
        Columns.push({
            df: "PID", c: 'Cấp cha', vi: 4, fvi: 4, v: AccessRights.danhmuc.PID != AccessRights.danhmuc.ID, fv: true, lds: dataMainP.data, lve: 'ID', lde: 'TEN',
            ops: {
            ...
            vr: [
                {
                    type: 'async',
                    message: 'Cấp cha không phù hợp',
                    ignoreEmptyValue: true,
                    validationCallback: async (e) => {
                        const { value, data } = e;
                        if (data?.id == null || value === 0) {
                            return true; // Thêm mới hoặc chưa chọn gì
                        }
                        if (value === data.id) {
                            return false; // Không được chọn chính nó làm cấp cha
                        }
                        return true;
                    }
        ...
        if (c.KIEUDULIEU == 'file') {
            ...
            if (!CheckEmpty(c.DM_ID)) {
                ...
                if (AccessRights.DmTree.find(f => f.ID == c.DM_ID)) {
                    // cc.ect = function (cellElement, cellInfo) {
                    //     if (cellInfo.cellElement == null) {
                    //         var ops = GlobalService.opsGridDropdownTree(cellInfo, {
                    //             cDrop: {
                    //                 dataSource: dtDanhMuc.data,
                    //             },
                    //             cTree: {
                    //                 rootValue: 0,
                    //                 searchEnabled: true,
                    //             }
                    //         });
                    //         $("<div>").appendTo(cellElement).dxDropDownBox(ops);
                    //     }
                    // };
                }
...
watch(isReady, (ready) => {
    if (ready) {
        nextTick(() => {
            const instance = baseTreeRef.value?.instance
            if (instance) {
                if (AccessRights.danhmuc.CHIEURONG) instance.option('editing.popup.width', function () { return window.innerWidth > AccessRights.danhmuc.CHIEURONG ? AccessRights.danhmuc.CHIEURONG : window.innerWidth * 0.9; });
            } else {
